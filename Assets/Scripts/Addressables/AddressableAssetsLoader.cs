    ...
    public class AddressableAssetsLoader<T> : BaseAddressableLoader where T : Object
    {
        ...
        public override async UniTask Load()
        {
            ...
            try
            {
                ...
                {
                    AddressablesManager.Log($"Succeed to load assets with key: {Key}");
                    Status = EAddressableOperationStatus.Success;
                }
                else
                {
                    AddressablesManager.LogError($"Failed to load assets with key: {Key}");
        ...
        public List<T> GetResult() => Status != EAddressableOperationStatus.Success ? null : _handle.Result.ToList();
    }
}