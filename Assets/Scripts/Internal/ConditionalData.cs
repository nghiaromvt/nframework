...
namespace NFramework.Internal
{
	...
	{
		public bool IsSet => !_fieldToCheck.IsNullOrEmpty() || !_fieldsToCheckMultiple.IsNullOrEmpty() || !_predicateMethod.IsNullOrEmpty();
		...
		{
			if (!_fieldToCheck.IsNullOrEmpty()) yield return (_fieldToCheck, _inverse, _compareValues);
			if (!_fieldsToCheckMultiple.IsNullOrEmpty())
			{
