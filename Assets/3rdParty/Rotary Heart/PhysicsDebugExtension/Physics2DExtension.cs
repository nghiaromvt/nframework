    ...
    public static partial class Physics2D
    {
            ...
            PreviewCondition preview = PreviewCondition.None, float drawDuration = 0, Color? hitColor = null, Color? noHitColor = null, bool drawDepth = false, CastDrawType drawType = CastDrawType.Minimal)
        {
            ...
            if (preview != PreviewCondition.None)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    DebugExtensions.DebugOneSidedCapsule(point1 + direction * hit.distance, point2 + direction * hit.distance, (hitColor ?? Color.green),
            ...
            PreviewCondition preview = PreviewCondition.None, float drawDuration = 0, Color? hitColor = null, Color? noHitColor = null, bool drawDepth = false)
        {
            ...
            if (preview != PreviewCondition.None)
            {
                ...
                {
                    Debug.DrawLine(start, end, collided ? (hitColor ?? Color.green) : (noHitColor ?? Color.red), drawDuration);
                }

                if (preview == PreviewCondition.Game || preview == PreviewCondition.Both)
                {
                    GLDebug.DrawLine(start, end, collided ? (hitColor ?? Color.green) : (noHitColor ?? Color.red), drawDuration);
                }
            }
            ...
            PreviewCondition preview = PreviewCondition.None, float drawDuration = 0, Color? hitColor = null, Color? noHitColor = null, bool drawDepth = false)
        {
            ...
            if (preview != PreviewCondition.None)
            {
                ...
                if (preview == PreviewCondition.Editor || preview == PreviewCondition.Both)
                {
                    Debug.DrawLine(previewOrigin, origin + direction * distance, collided ? (hitColor ?? Color.green) : (noHitColor ?? Color.red), drawDuration);
                }

                if (preview == PreviewCondition.Game || preview == PreviewCondition.Both)
                {
                    GLDebug.DrawLine(previewOrigin, origin + direction * distance, collided ? (hitColor ?? Color.green) : (noHitColor ?? Color.red), drawDuration);
                }
            }
